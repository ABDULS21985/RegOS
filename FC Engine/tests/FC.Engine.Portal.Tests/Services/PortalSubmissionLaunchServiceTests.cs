...
public class PortalSubmissionLaunchServiceTests
{
    ...
    [Fact]
    public async Task ResolvePrimaryBulkSubmitAsync_Uses_Entitled_Module_And_Return()
    {
        var tenantId = Guid.NewGuid();
        var entitlementService = new Mock<IEntitlementService>();
        entitlementService
        ...
        var result = await sut.ResolvePrimaryBulkSubmitAsync();

        result.Href.Should().Be("/submit/bulk?module=CAPITAL_SUPERVISION&returnCode=CAP_BUF");
        result.ModuleCode.Should().Be("CAPITAL_SUPERVISION");
        result.ReturnCode.Should().Be("CAP_BUF");
    }

    [Fact]
    public async Task ResolvePrimaryBulkSubmitAsync_Falls_Back_To_Generic_Bulk_When_Tenant_Context_Is_Missing()
    {
        var entitlementService = new Mock<IEntitlementService>(MockBehavior.Strict);
        var templateCache = new Mock<ITemplateMetadataCache>(MockBehavior.Strict);
        ...
        result.Href.Should().Be("/submit/bulk");
        result.ModuleCode.Should().BeNull();
        result.ReturnCode.Should().BeNull();
    }

    [Fact]
    public async Task ResolvePrimarySubmissionsAsync_Uses_Module_Context()
    {
        var tenantId = Guid.NewGuid();
        var entitlementService = new Mock<IEntitlementService>();
        entitlementService
        ...
        templateCache
            ...
            .ReturnsAsync(
            [
                CreateTemplate("OPS_IBM", "Important Business Services", "OPS_RESILIENCE", 44)
            ]);
        ...
        var result = await sut.ResolvePrimarySubmissionsAsync();

        result.Href.Should().Be("/submissions?module=OPS_RESILIENCE");
        result.ModuleCode.Should().Be("OPS_RESILIENCE");
        result.ReturnCode.Should().Be("OPS_IBM");
    }

    [Fact]
    public async Task ResolvePrimaryExtensionRequestAsync_Uses_Module_And_Return_Context()
    {
        var tenantId = Guid.NewGuid();
        var entitlementService = new Mock<IEntitlementService>();
        entitlementService
        ...
        var result = await sut.ResolvePrimaryExtensionRequestAsync();

        result.Href.Should().Be("/calendar?module=CAPITAL_SUPERVISION&returnCode=CAP_BUF&requestExtension=true");
        result.ModuleCode.Should().Be("CAPITAL_SUPERVISION");
        result.ReturnCode.Should().Be("CAP_BUF");
    }
